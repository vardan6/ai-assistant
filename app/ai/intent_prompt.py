...
- "B" (statistics & trends): multi-row aggregation/comparison over generation, weather, alerts, anomalies, maintenance, and plant metadata — average daily yield, total_yield energy, performance-ratio ranking, AC power, maintenance cost, maintenance duration, anomaly power loss, mean-time-to-resolve, highest feed-in tariff, worst-performing plant by performance ratio.
...
- metric: the quantity asked for if any ("daily_yield", "performance_ratio", "total_yield", "tariff_usd_per_kwh", "weather", "anomalies", "power_loss", "mttr", "maintenance_cost", "maintenance_duration", "ac_power", "downtime", ...), else null.
- out_of_scope: true ONLY if the dataset cannot answer it (e.g. revenue lost from downtime — no revenue data exists). Plant feed-in tariff, maintenance cost/duration, anomaly estimated power loss, and AC power are in scope.
- Maintenance duration questions should map to "maintenance_duration", not alert "mttr".
- "Full health summary" questions combine current status, derived metrics, and anomalies, so classify them as types ["A", "B", "C"] unless the user narrows the scope.
